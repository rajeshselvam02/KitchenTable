...
async function getOrCreateSession(phone) {
    const { rows } = await db_1.default.query(`INSERT INTO whatsapp_sessions (phone) VALUES ($1)
     ON CONFLICT (phone) DO UPDATE SET updated_at = NOW()
     RETURNING *`, [phone]);
    return rows[0];
}
async function updateSession(phone, state, data) {
    await db_1.default.query(`UPDATE whatsapp_sessions SET state = $1, data = $2, updated_at = NOW() WHERE phone = $3`, [state, JSON.stringify(data), phone]);
}
async function handleMessage(phone, message) {
    ...
    switch (session.state) {
        ...
        case 'show_plans': {
            ...
            const planMap = { '1': 'starter', '2': 'weekly', '3': 'monthly' };
        ...
        case 'get_meal_type': {
            ...
            const mealMap = { '1': 'lunch', '2': 'dinner', '3': 'both' };
        ...
        case 'confirm': {
            ...
            const { rows: custRows } = await db_1.default.query(`
      ...
      `, [data.name, phone, data.address]);
            ...
            if (!customerId) {
                const { rows } = await db_1.default.query(`SELECT id FROM customers WHERE phone = $1`, [phone]);
      ...
      `, [customerId, data.plan, mealType, startDate, endDate, planInfo.price, phone]);
            ...
            if (data.plan === 'starter') {
                const { rows: subRows } = await db_1.default.query(`SELECT id FROM subscriptions WHERE customer_id = $1 ORDER BY id DESC LIMIT 1`, [customerId]);
                await db_1.default.query(`
