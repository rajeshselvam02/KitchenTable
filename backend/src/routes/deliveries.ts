...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const subId = req.query.subscription_id ? parseInt(req.query.subscription_id as string) : null;
...
router.post('/:id/porter', async (req: Request, res: Response) => {
  try {
    const { rows } = await pool.query(`
    ...
    const porterData = await porterRes.json() as any;
...
router.post('/:id/skip', async (req: Request, res: Response) => {
  try {
    const { skipDelivery } = await import('../services/scheduler');
