...
async function generateDeliveries(date: string) {
  try {
    ...
    for (const sub of subs) {
      ...
      for (const meal of meals) {
        const { rows: menuRows } = await pool.query(
          `SELECT dish_id FROM menus WHERE day = $1 LIMIT 1`, [date]
        );
...
async function updateRawMaterials(date: string) {
  try {
...
export async function skipDelivery(deliveryId: number, reason: string, comment: string) {
...
async function autoConfirmDeliveries() {
  ...
  try {
    if (hour >= 8) {
      ...
      `, [today]);
      if (rowCount) console.log('[Scheduler] Auto-confirmed ' + rowCount + ' lunch deliveries');
    }
    if (hour >= 14) {
      const { rowCount } = await pool.query(`
        UPDATE deliveries SET status = 'auto_confirmed', confirmed_at = NOW()
      ...
      `, [today]);
      if (rowCount) console.log('[Scheduler] Auto-confirmed ' + rowCount + ' dinner deliveries');
    }
  } catch (err) {
...
export function startScheduler() {
  ...
  cron.schedule('30 8 * * *', autoConfirmDeliveries, { timezone: 'Asia/Kolkata' });
  cron.schedule('30 14 * * *', autoConfirmDeliveries, { timezone: 'Asia/Kolkata' });
  cron.schedule('30 5 * * *', () => {
...
export { generateDeliveries, updateRawMaterials };
